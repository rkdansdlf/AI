...
def list_columns():
    ...
    try:
        ...
        cur.execute("SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'player_season_pitching'")
        columns = cur.fetchall()
        print("Columns in player_season_pitching:")
        for col in columns:
            print(f"- {col[0]} ({col[1]})")
            
        cur.execute("SELECT quality_starts, COUNT(*) FROM player_season_pitching WHERE season = 2025 GROUP BY quality_starts")
