...
async def debug_search(
    ...
    # 결과 품질 분석
    similarity_scores = [
        doc.get("similarity", 0) for doc in docs if doc.get("similarity")
    ]
    avg_similarity = (
    ...
    unique_tables = set(
        doc.get("source_table") for doc in docs if doc.get("source_table")
    )
        ...
        "comparison": {
            "single_query": {
                ...
                "avg_similarity": round(
                    (
                        sum(single_similarities) / len(single_similarities)
            ...
            "multi_query": {
                ...
                "avg_similarity": round(
                    (
                        sum(multi_similarities) / len(multi_similarities)
                        if multi_similarities
                        else 0
                    ),
                    4,
                ),
                "results_preview": [
                    doc.get("title", doc.get("content", "")[:100])
                    for doc in docs_multi[:3]
                ],
            },
        },
        "recommendation": (
            "multi_query" if len(docs_multi) > len(docs_single) else "single_query"
        ),
    }
