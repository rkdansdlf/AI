...
class RegulationQueryTool:
    ...
    def __init__(self, connection: PgConnection):
        ...
        self.regulation_categories = {
            "basic": ["리그구성", "경기시간", "이닝", "타이브레이크", "지명타자", "비디오판독"],
            "player": ["선수등록", "외국인선수", "FA", "드래프트", "트레이드", "웨이버"],
            "game": ["경기진행", "투구시계", "방해", "보크", "홈런", "인필드플라이", "우천"],
            "technical": ["기록", "안타", "실책", "승리투수", "세이브", "홀드", "심판"],
            "discipline": ["징계", "폭력", "도박", "약물", "처벌", "벌금", "출장정지"],
            "postseason": ["포스트시즌", "플레이오프", "와일드카드", "한국시리즈", "4승어드밴티지"],
    ...
    def get_regulation_by_category(self, category: str, limit: int = 10) -> Dict[str, Any]:
        """
        특정 카테고리의 규정들을 조회합니다.
        ...
        try:
            ...
            if rows:
                ...
                result["found"] = True
                result["total_found"] = len(rows)
                logger.info(f"[RegulationQuery] Found {len(rows)} regulations for category {category}")
            else:
                logger.warning(f"[RegulationQuery] No regulations found for category: {category}")
    ...
    def validate_regulation_reference(self, regulation_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if row:
                ...
                logger.warning(f"[RegulationQuery] Regulation not found: {regulation_code}")
