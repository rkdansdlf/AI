...
def fip(
    HR: int, BB: int, HBP: int, K: int, IP: float, ctx: LeagueContext
) -> Optional[float]:
...
def war_pitcher(fip_val: float, IP: float, ctx: LeagueContext) -> Optional[float]:
    ...
    try:
        runs_above_replacement = (ctx.lg_ra9 * ctx.park_factor - fip_val) * (IP / 9)
...
def grade_metric_ko(
    key: str, value: float, grades: Optional[LeagueGradeBoundaries] = None
) -> str:
    if value is None:
        return "데이터 부족"
    ...
    if k == "ERA":
        ...
        return "재정비 필요"
    if k == "WHIP":
        if v <= grades.whip_excellent:
            return "출루 억제 우수"
        if v <= grades.whip_good:
            return "양호"
        if v <= grades.whip_avg:
            return "평균권"
        return "불안정"
    if k == "OPS":
        if v >= grades.ops_elite:
            return "엘리트"
        if v >= grades.ops_top:
            return "상위권"
        if v >= grades.ops_avg:
        ...
        return "낮음"
    if k == "WRC+":
        if v >= grades.wrc_plus_mvp:
            return "MVP급"
        if v >= grades.wrc_plus_elite:
            return "엘리트"
        if v >= grades.wrc_plus_top:
            return "상위권"
        if v >= grades.wrc_plus_avg:
            return "평균권"
        if v >= grades.wrc_plus_low:
            return "하락세"
        return "저조"
    if k == "WAR":
        if v >= grades.war_mvp:
            return "MVP 후보"
        if v >= grades.war_allstar:
            return "올스타급"
        if v >= grades.war_starter:
            return "선발 레귤러급"
        if v >= grades.war_role:
            return "롤플레이어"
        return "대체선수 이하"
    if k == "FIP":
        if v <= grades.fip_ace:
        ...
        return "재정비 필요"
    if k == "K/9":
        if v >= grades.k_per_9_excellent:
            return "삼진 능력 탁월"
        if v >= grades.k_per_9_good:
            return "양호"
        if v >= grades.k_per_9_avg:
            return "평균권"
        return "낮음"
    if k == "BB/9":
        if v <= grades.bb_per_9_excellent:
            return "제구 매우 안정"
        if v <= grades.bb_per_9_good:
            return "양호"
        if v <= grades.bb_per_9_avg:
            return "평균권"
        return "불안정"
    if k == "ERA-" or k == "FIP-":
        if v <= 75:
...
def describe_metric_ko(
    key: str,
    value: float,
    precision: int = 2,
    grades: Optional[LeagueGradeBoundaries] = None,
) -> str:
...
def csw_rate(
    called_strikes: int, swinging_strikes: int, total_pitches: int
) -> Optional[float]:
...
def pitcher_rank_score(era_minus_v, fip_minus_v, kbb_pct, whip, ip):
    ...
    base = (
        0.4 * era_minus_v + 0.3 * fip_minus_v + 0.2 * (100 - kbb_pct) + 0.1 * whip * 100
    )
