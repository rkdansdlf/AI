...
class ContextFormatter:
    ...
    def format_context(
        ...
        if intent == "award_lookup" or entity_filter.award_type:
            return self._format_award_context(
        ...
        if intent == "movement_lookup" or entity_filter.movement_type:
            return self._format_player_movement(
                processed_data, query, entity_filter, year
            )

        # 기존 의도들
        if intent == "stats_lookup" or entity_filter.stat_type:
            return self._format_statistical_ranking(
                processed_data, query, entity_filter, year
            )
        elif intent == "player_profile" or entity_filter.player_name:
            return self._format_player_profile(
                processed_data, query, entity_filter, year
            )
        elif intent == "team_analysis" or entity_filter.team_id:
            return self._format_team_analysis(
                processed_data, query, entity_filter, year
            )
        elif intent == "comparison":
            return self._format_comparison_table(
                processed_data, query, entity_filter, year
            )
    ...
    def _format_statistical_ranking(
        ...
        if entity_filter.stat_type:
            sp_pitchers = self._sort_by_requested_stat(
                sp_pitchers, entity_filter.stat_type, "pitcher"
            )
            rp_pitchers = self._sort_by_requested_stat(
                rp_pitchers, entity_filter.stat_type, "pitcher"
            )
            batters = self._sort_by_requested_stat(
                batters, entity_filter.stat_type, "batter"
            )

        if sp_pitchers:
            header = kbo_metrics.scope_header(
                year, len(set(p["team"] for p in sp_pitchers)), "SP", self.MIN_IP_SP
            )
        ...
        if rp_pitchers:
            header = kbo_metrics.scope_header(
                year, len(set(p["team"] for p in rp_pitchers)), "RP", self.MIN_IP_RP
            )
        ...
        if batters:
            header = kbo_metrics.scope_header(
                year, len(set(b["team"] for b in batters)), "BAT", self.MIN_PA_BATTER
            )
            context_parts.append(f"\n### 타자 성적\n{header}\n")
    ...
    def _format_player_profile(
        ...
        if not player_data:
            ...
            context_parts.append(
                f"'{target_player}' 선수의 {year}년 상세 기록 데이터를 찾을 수 없습니다."
            )
            context_parts.append("")
            context_parts.append("**가능한 원인:**")
            context_parts.append("- 선수명 오타 또는 다른 표기법")
            context_parts.append(
                "- 해당 연도에 최소 출전 기준 미달 (타자 100타석, 투수 30이닝 미만)"
            )
        ...
        # 선수 기본 정보
        context_parts.append(
            f"### {player_data['name']} ({player_data['team']}) - {year}년 성적"
        )
        ...
        if player_data in processed_data["pitchers"]:
            ...
            context_parts.append(
                f"**이닝**: {kbo_metrics.format_ip(player_data['ip'])} IP"
            )
            context_parts.append(f"**방어율**: {player_data['era']:.2f}")
            context_parts.append(
                f"**WHIP**: {kbo_metrics.describe_metric_ko('WHIP', player_data['whip'])}"
            )
            context_parts.append(
                f"**ERA-**: {kbo_metrics.describe_metric_ko('ERA-', player_data['era_minus'], 0)}"
            )
            context_parts.append(
                f"**FIP-**: {kbo_metrics.describe_metric_ko('FIP-', player_data['fip_minus'], 0)}"
            )
            context_parts.append(
                f"**K-BB%**: {kbo_metrics.describe_metric_ko('K-BB%', player_data['kbb_pct'], 1)}%"
            )
        else:
            ...
            if player_data.get("avg"):
                context_parts.append(
                    f"**타율(AVG)**: {kbo_metrics.describe_metric_ko('AVG', player_data['avg'], 3)}"
                )

            # 출루율/장타율
            if player_data.get("obp"):
                context_parts.append(
                    f"**출루율(OBP)**: {kbo_metrics.describe_metric_ko('OBP', player_data['obp'], 3)}"
                )
            if player_data.get("slg"):
                context_parts.append(
                    f"**장타율(SLG)**: {kbo_metrics.describe_metric_ko('SLG', player_data['slg'], 3)}"
                )

            # 종합 지표
            if player_data.get("ops"):
                context_parts.append(
                    f"**OPS**: {kbo_metrics.describe_metric_ko('OPS', player_data['ops'], 3)}"
                )
            ...
            if player_data.get("war"):
                context_parts.append(
                    f"**WAR**: {kbo_metrics.describe_metric_ko('WAR', player_data['war'], 2)}"
                )
    ...
    def _format_team_analysis(
        ...
        context_parts = []
        context_parts.append(
            f"### {self._get_full_team_name(target_team)} {year}년 주요 선수"
        )

        # 팀 소속 선수들만 필터링
        team_pitchers = [
            p
            for p in processed_data["pitchers"]
            if p["team"] == self._get_full_team_name(target_team)
        ]
        team_batters = [
            b
            for b in processed_data["batters"]
            if b["team"] == self._get_full_team_name(target_team)
        ]
    ...
    def _sort_by_requested_stat(
        self, players: List[Dict], stat_type: str, position: str
    ) -> List[Dict]:
        ...
        if stat_type in sort_mapping:
            ...
            try:
                return sorted(
                    players,
                    key=lambda p: p.get(key, 0 if reverse else 999),
                    reverse=reverse,
                )
    ...
    def _format_pitcher_line(
        self, pitcher: Dict, rank: int, focus_stat: Optional[str] = None
    ) -> str:
        """투수 한 줄 요약을 포맷합니다."""
    ...
    def _format_batter_line(
        self, batter: Dict, rank: int, focus_stat: Optional[str] = None
    ) -> str:
        """타자 한 줄 요약을 포맷합니다."""
    ...
    def _format_award_context(
        ...
        if entity_filter.award_type and entity_filter.award_type != "any":
            award_name = award_type_display.get(
                entity_filter.award_type, entity_filter.award_type
            )
    ...
    def _format_player_movement(
        ...
        if entity_filter.movement_type and entity_filter.movement_type != "any":
            movement_name = movement_type_display.get(
                entity_filter.movement_type, entity_filter.movement_type
            )
        ...
        if movements:
            ...
            for movement in movements:
                m_type = movement.get("section") or movement.get(
                    "movement_type", "기타"
                )
